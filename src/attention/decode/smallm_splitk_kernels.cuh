...
__global__ void attention_decode_bf16_multistage_ws_smallm_splitk_kernel(
  ...
  // TMA head tile index for Y store (valid only when heads_per_group == kTileN).
...
#pragma unroll
      ...
      // lse_batch base can be misaligned when num_head_q is odd (e.g. ichunk=1
      // gives offset ichunk*num_head_q which is odd, making the address not
      // 8-byte aligned). Use scalar stores whenever the base is not aligned.

...
def parse_args():
    ...
    p.add_argument(
        "--warmup", type=int, default=5, help="Warmup iterations before graph capture"
    )
    p.add_argument(
        "--iters", type=int, default=100, help="Graph replay iterations for timing"
    )
    return p.parse_args()
...
BATCH_SIZES = [i for i in range(512, 15873, 512)]
...
def make_inputs(
    batch_size, hidden_size, intermediate_size, num_experts_local, topk, device="cuda"
):
    """
    ...
    gate_up_weight = torch.randn(
        (num_experts_local, intermediate_size * 2, hidden_size),
        dtype=dtype,
        device=device,
    )
...
def bench_sglang(
    ...
    cumsum_buf = torch.empty(
        (num_experts_local + 2,), dtype=torch.int32, device=x.device
    )
    ...
    cache3 = torch.empty(
        (batch_size, topk, hidden_size), dtype=torch.bfloat16, device=x.device
    )
...
def main():
    ...
    print(f"\nDevice              : {prop.name}")
    print(
        f"Model config        : hidden={hidden_size}, inter={intermediate_size}, "
        f"experts={num_experts}, topk={topk}, tp={tp_size}"
    )
    print(f"Local experts/GPU   : {num_experts_local}")
    print(
        f"Weight shapes       : gate_up=[{num_experts_local}, {intermediate_size * 2}, {hidden_size}], "
        f"down=[{num_experts_local}, {hidden_size}, {intermediate_size}]"
    )
    print(
        f"Timing              : warmup={args.warmup}, iters={args.iters} (CUDA graph replay)\n"
    )

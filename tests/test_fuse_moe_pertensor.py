...
def naive_group_gemm(x, w, cu_seqlens, scale, expert_ids):
    ...
    for i in range(num_group):
        ...
        y_group = (
            (x_group.to(torch.float32) @ w_group.t().to(torch.float32))
            * scale[i].to(torch.float32)
        ).to(torch.bfloat16)
...
def naive_reduce(x_bf16, topk_pos, topk_scale, shared_output=None):
    ...
    y_bf16 = torch.zeros(
        (num_seq, hidden_size), dtype=torch.bfloat16, device=x_bf16.device
    )
...
def naive_fuse_moe_pertensor_fp8(
    ...
    # count_and_gather
    gate_up_input, topk_pos, seqlens, cu_seqlens, expert_ids = (
        naive_gather_expert_inputs(x, topk_ids, num_expert, rank_ep)
    )
    ...
    down_output = naive_group_gemm(
        down_input, down_weight, cu_seqlens, down_scale, expert_ids
    )
...
def test_fuse_moe_pertensor_fp8(
    ...
    topk_ids = torch.randint(
        0, num_expert, (num_seq, num_topk), dtype=torch.int32, device="cuda"
    )
    topk_ids, _ = torch.sort(topk_ids, dim=1)

    x = (
        torch.randn((num_seq, hidden_size), dtype=torch.float, device="cuda") / 100
    ).to(dtype)
    ...
    gate_up_scale = torch.randn(
        (num_expert // size_ep), dtype=torch.float, device="cuda"
    )
    ...
    if has_shared_output:
        shared_output = torch.randn(
            (num_seq, hidden_size), dtype=torch.bfloat16, device="cuda"
        )
    else:
        shared_output = None

    # Allocate workspace
    ws_size = hpc.get_workspace_size_pertensor_fp8(
        max_num_tokens=num_seq,
    ...
    output = torch.empty(num_seq, hidden_size, dtype=torch.bfloat16, device="cuda")

    hpc.fuse_moe_pertensor_fp8(
        x,
        gate_up_weight,
        down_weight,
        gate_up_scale,
    ...
    assert allclose(gt.to(torch.float32), output.to(torch.float32), rtol=0.08, atol=0.1)
